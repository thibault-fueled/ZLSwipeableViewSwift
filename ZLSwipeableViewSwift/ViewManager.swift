...
class ViewManager : NSObject {
	...
	init(view: UIView, containerView: UIView, index: Int, miscContainerView: UIView, animator: UIDynamicAnimator, swipeableView: ZLSwipeableView) {
		...
		super.init()
		let pan = ZLPanGestureRecognizer(target: self, action: #selector(ViewManager.handlePan(_:)))
		pan.cancelsTouchesInView = false
		view.addGestureRecognizer(pan)
		
		let tap = ZLTapGestureRecognizer(target: self, action: #selector(ViewManager.handleTap(_:)))
